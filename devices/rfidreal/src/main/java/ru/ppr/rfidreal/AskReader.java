 ...
 * @author G.Kashka
 */
public class AskReader implements IAscReader {

    private static final String TAG = AscReader.class.getSimpleName();
    public static final String TAG2 = "Sam 2 eSmart";
    /**
    ...
    public AskReader(Context context) {
        Logger.info(TAG, "AscReader() constructor START");
        final CountDownLatch countDownLatch = new CountDownLatch(1);

        final CountDownLatch countDownLatch1 = new CountDownLatch(1);
        BReader.getInstance(context, new IOnGetReaderInstanceListener() {
            @Override
            public void OnGetReaderInstance(BReader reader) {
...
/*    @Override
    public int initSam2() {
        ...
        Log.d(TAG2, "Firmware version: " + sb.toString());
        // Powers on SAM
        ...
        // Resets SAM 1
        mReader.cscSelectSam((byte)1, Defines.SAM_PROT_ISO_7816_T0);
        byte[] atr = new byte[256];
        int[] atrLength = new int[1];
        mReader.cscResetSam((byte) 0x01, atr, atrLength);
        Log.d(TAG2, "SAM ATR: " + CpcBytes.byteArrayToString(atr, atrLength[0]));
        setPower(true);

        byte[] dataToSend = CpcBytes.parseHexStringToArray("00A40800068F017F015003");
        Log.d(TAG2, "Data to send: " + CpcBytes.byteArrayToString(dataToSend));
        byte[] dataReceived = new byte[256];
        int[] dataReceivedLength = new int[1];

        int res = mReader.cscIsoBCommandsam(dataToSend, dataToSend.length, dataReceived, dataReceivedLength);
...
//        int res = mReader.cscVersionCsc(firmwareVersion);
    ...
    private int cscVersionCscDecompiled(StringBuilder sb) {
        ...
        int ret = cscSendReceive(timeout, BCommands.giCSCTrame, BCommands.giCSCTrameLn);
    ...
    public boolean cscSearchCardExt(byte[] com, int[] lpcbATR, byte[] lpATR) {
        ...
        int mask =  Defines.SEARCH_MASK_ISOA | Defines.SEARCH_MASK_MIFARE | Defines.SEARCH_MASK_MONO | Defines.SEARCH_MASK_MV4K
                | Defines.SEARCH_MASK_MV5K | Defines.SEARCH_MASK_TICK;

        byte forget = (byte) 0x01;
        byte timeOut = (byte) 0xFF;

        lastResult = mReader.cscSearchCardExt(search, mask, forget, timeOut, com, lpcbATR, lpATR);
    ...
    private byte[] getCardAtrHistoricalBytes(byte[] atr) {
        ...
        if ((null == atr) || (atr.length < 3)) {
            ...
            if(historicalBytesIndex > 0 && historicalBytesLength > 0)
